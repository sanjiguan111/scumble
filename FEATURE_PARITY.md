...
| **Line**                                       | ✅     | `<Line x1 y1 x2 y2>` → `scumble-line`; stroke-only (defaults `style="stroke"`, explicit fill ignored natively)                                                                                                                                          |
...
> `parsePoints` (SVG `points` string → `{x,y}` pairs) now lives in `@scumble/graphics` alongside the other string parsers.
...
| **Text / Paragraph**                                                 | ✅     | `<Paragraph width>` + `<TextSpan>` children — styled spans serialize to base64 `SpanList` bytes on the `spans` prop (text + styles only; glyphs never cross JS). Layout runs in the TASM measure pass — iOS CoreText, Android HarfBuzz + skity `FontManager` fallback + a shared space/CJK-kinsoku line breaker — so the measured height feeds Lynx layout synchronously; `onLayout` (async `"layout"` LynxDetailEvent) carries `{height, lineCount}`. The laid-out glyph runs ride the **extra-bundle side channel** next to the command batch (`ParagraphRunList`, node-id keyed, entry-level idempotent overwrite), fonts cross threads through the shared `FontRegistry`, and the renderer draws one `DrawGlyphs` per run (glyph atlas handles the rest). `textAlign` (left/center/right + **justify**, shipped 2026-09-20: inter-word spaces only — the line's slack splits across its interior spaces, CoreText/SkParagraph semantics; last line, ellipsized lines, and lines without spaces stay left, slack is never negative; iOS rides `kCTTextAlignmentJustified`, Android distributes via the shared host-tested `JustifyLineSlack` in `paragraph_shaper.cc`), `lineHeight` multiplier, `maxLines` + ellipsis, custom fonts by URI (inline `data:` base64 ttf/otf, or schemed `http(s)`/`file`/host sources via a host-injectable loader — a miss falls back to the default font, bytes landing asynchronously re-trigger layout), and node-level gradient fills + color filters through the glyph-atlas paint, and `direction` (ltr/rtl/auto) BiDi reordering — CoreText's built-in UAX #9 on iOS, SheenBidi statically linked + per-bidi-run HarfBuzz shaping on Android. **Differences vs RN-Skia**: no imperative `ParagraphBuilder`/JSI — layout is native by necessity (public Lynx Android SDK compiles NAPI off); justification stretches inter-word spaces only (no inter-ideograph CJK stretching — lines without spaces stay left); image-shader fills and blur filters on text are ignored by skity's glyph pipeline (upstream). **Text decoration ships** (2026-09-02): `decoration` (bitfield: underline/overline/line-through, combinable), `decorationColor`, `decorationThickness`, `decorationStyle` (solid/double/dotted/dashed/wavy) on `<TextSpan>` + paragraph-level span defaults. Geometry is computed at LAYOUT time on both backends (one `TextDecorationRun` per span × line × set bit, x/width from the same pen positions as the glyphs, y/thickness from the run font's metrics — SkParagraph semantics, incl. per-span metrics on mixed-size lines) and rides the ParagraphRunList side channel; the renderer strokes it after the glyphs through the same paint lane (node COLOR/GRADIENT fill applies to decorations too). `decorationThickness` is ABSOLUTE px (0/unset = the font's metric thickness) — a deliberate deviation from RN-Skia's multiplier semantics. See [TEXT_PARAGRAPH_DESIGN.md](TEXT_PARAGRAPH_DESIGN.md) + RENDER_ARCHITECTURE.md §13 |
...
5. ~~**No text decoration on `<TextSpan>`**~~ — **fixed (2026-09-02)**:
   ...
   runs carry no advances and iOS pos_y mixes per-glyph offsets, so the
   renderer could never reconstruct x/y) — one TextDecorationRun per
   span × line × set bit, computed from the same pen positions the glyphs
   render at and the run font's underline/strikeout metrics (skity
   FontMetrics, filled by both scalers), serialized on the ParagraphRunList
   side channel and stroked by the renderer after the glyphs (solid/double
   rects, dotted/dashed dash lines, wavy quad path — SkParagraph's shapes).
   The only semantic deviation: thickness is absolute px, not RN-Skia's
   multiplier. See RENDER_ARCHITECTURE.md §13.
...
11. ~~**Animation path investigation** (F.3)~~ — investigated 2026-08-21,
    ...
    - **11a. Stage 1: invoke-driven animated values** (§G.2, §G.3 verified
      PASS) — shared-value API in `@scumble/react`; worklet lane where
...
14. **Small-items bundle** (F.1.4 + justification) — ~~per-corner radii~~
    (done 2026-09-18, F.1.4), ~~paragraph justification~~ (done 2026-09-20,
    `textAlign: "justify"` — inter-word spaces only; iOS
    `kCTTextAlignmentJustified`, Android `JustifyLineSlack` slack
    distribution in the shaper, host-tested `paragraph_justify_test.cc`),
    antiAlias toggle, DropShadow variants.
