...
- The `cocoapods-lynx-library` gem is pinned `~> 4.0.x` in `packages/example/ios/Gemfile` to track the SDK's version line — the 4.1 plugin line ships alongside the harmony-only SDK 4.1.0 that iOS never got, so an unconstrained `>=` resolves straight to 4.1.0 (matching the Android plugin pin in `settings.gradle.kts`).
