...
- **F.1 same name, different semantics** (fixable, touches the command stream): single paint slot (see [[rendering#Paint: double pass, single slot]]); minor items — no per-corner ClipRRect radii, antiAlias hard-wired true, DropShadow lacks inner/shadowOnly. Two former members were retracted/fixed: nested transforms DO cascade (the old claim was a documentation misreading); group opacity went exact via the saveLayer lane.
