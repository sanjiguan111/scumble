...
if (!existsSync(flatc)) {
  ...
  console.warn('[lynx-skity] Run `tools/hab sync` first, then `pnpm --filter lynx-skity generate-fbs`.');
  // Non-fatal: an install must not break just because habitat hasn't synced yet.
  process.exit(0);
}

const fbsFiles = ['render_tree_common.fbs', 'render_tree_style.fbs', 'render_tree.fbs']
...
execFileSync(
  flatc,
  ['--java', '--java-package-prefix', 'com.skity.graphics',
   '-o', javaOut, '-I', schemaDir, ...fbsFiles],
  { stdio: 'inherit' });
