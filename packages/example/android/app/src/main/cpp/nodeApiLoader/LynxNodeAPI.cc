...
std::vector<std::string> BuildAddonCandidates(const std::string& library_name) {
  ...
  const bool has_ext = EndsWith(library_name, ".node") ||
                       EndsWith(library_name, ".so") ||
                       EndsWith(library_name, ".dylib") ||
                       EndsWith(library_name, ".dll");
  if (has_path) {
    if (!has_ext) {
...
bool LynxNodeAPI::LoadNodeAddon(NodeAddon& addon,
  ...
  for (const auto& path : candidates) {
    ...
    PosixLoader::Symbol initFn =
        PosixLoader::getSymbol(library, "napi_register_module_v1");
    if (initFn != nullptr) {
      addon.init = initFn;
      return true;
    }
  ...
  if (last_error != nullptr) {
    std::fprintf(stderr,
                 "Failed to load Node Addon '%s'. Last dlopen/dlsym error: %s\n",
...
bool LynxNodeAPI::InitializeNodeModule(void* env_ptr, NodeAddon& addon) {
  ...
  napi_status status = napi_get_named_property(env, global,
                                               "__lynx_node_addon_exports__",
                                               &exports_obj);
  ...
  if (type != napi_object) {
    ...
    if (napi_set_named_property(env, global, "__lynx_node_addon_exports__",
                                exports_obj) != napi_ok)
      return false;
  }
