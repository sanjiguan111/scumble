...
// ScumbleImageLoaderRegistry.requestImage(_:) when a node first carries a uri;
// pixels land in the shared ImageStore from the render queue and every live
// session redraws (see ScumbleImageLoader.mm).
...
@interface ScumbleImagePixels : NSObject
...
/// via ScumbleImageLoaderRegistry.setImageLoader:.
@protocol ScumbleImageLoader <NSObject>
...
@interface ScumbleImageLoaderRegistry : NSObject
