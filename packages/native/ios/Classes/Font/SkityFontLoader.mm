...
// weak session table).
@interface SkityFontWaiter : NSObject
@property(nonatomic, weak, nullable) LynxUIContext *uiContext;
@property(nonatomic, assign) NSInteger sign;
@end

@implementation SkityFontWaiter
@end

@implementation SkityFontLoaderRegistry {
  id<SkityFontLoader> _loader;
  NSMutableSet<NSString *> *_pending;
  NSMutableDictionary<NSString *, NSMutableArray<SkityFontWaiter *> *> *_waiting;
}

+ (instancetype)shared {
  static SkityFontLoaderRegistry *registry;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{ registry = [[SkityFontLoaderRegistry alloc] init]; });
  return registry;
}
...
- (instancetype)init {
  self = [super init];
  if (self) {
    _loader = [[BuiltInSkityFontLoader alloc] init];
...
+ (void)setFontLoader:(nullable id<SkityFontLoader>)loader {
  SkityFontLoaderRegistry *registry = [SkityFontLoaderRegistry shared];
  @synchronized(registry) {
    registry->_loader = loader ?: [[BuiltInSkityFontLoader alloc] init];
  }
}
...
+ (void)requestFont:(NSString *)uri sign:(NSInteger)sign uiContext:(LynxUIContext *)uiContext {
  ...
  SkityFontLoaderRegistry *registry = [SkityFontLoaderRegistry shared];
  id<SkityFontLoader> loader;
  @synchronized(registry) {
    SkityFontWaiter *waiter = [[SkityFontWaiter alloc] init];
  ...
  [loader loadFont:uri
        onComplete:^(NSData *_Nullable bytes) {
          ...
          skityrt::TypefaceCache::Instance().StoreBytes(key, bytes != nil ? bytes.bytes : nullptr,
                                                        (size_t)bytes.length);
          NSArray<SkityFontWaiter *> *waiters;
          SkityFontLoaderRegistry *reg = [SkityFontLoaderRegistry shared];
          @synchronized(reg) {
          ...
          dispatch_async(dispatch_get_main_queue(), ^{
            for (SkityFontWaiter *waiter in waiters) {
              LynxUIContext *ctx = waiter.uiContext;
              if (ctx == nil) continue;
              [ctx findShadowNodeAndRunTask:waiter.sign
                                       task:^(LynxShadowNode *node) {
                                         if (![node isKindOfClass:[SkityNodeBase class]]) return;
                                         ((SkityNodeBase *)node).dirtyParagraph = YES;
                                         [node setNeedsLayout];
                                       }];
            }
          });
