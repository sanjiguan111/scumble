...
static int ScumbleAnimActionCode(NSString *name) {
...
@implementation ScumbleCanvasUI
...
- (UIView *)createView {
  ScumbleCanvasView *view = [[ScumbleCanvasView alloc] init];
...
- (void)onReceiveUIOperation:(id)extraData {
  ...
  if ([extraData isKindOfClass:[NSData class]]) {
    [(ScumbleCanvasView *)self.view consumeCommands:(NSData *)extraData];
    ...
    if (batch != nil || runs != nil) {
      [(ScumbleCanvasView *)self.view consumePayloadWithBatch:batch paragraphRuns:runs];
    }
  }
}

- (void)detachView {
  [(ScumbleCanvasView *)self.view destroySession];
  [super detachView];
}
...
LYNX_UI_METHOD(animateControl) {
  ...
  int action = actionName != nil ? ScumbleAnimActionCode(actionName) : -1;
  if (handle.length == 0 || action < 0) {
    callback(kUIMethodParamInvalid, nil);
    return;
  }
  double timeMs = [params[@"time"] doubleValue];
  ScumbleCanvasView *target = (ScumbleCanvasView *)self.view;
  if (![target isKindOfClass:[ScumbleCanvasView class]]) {
