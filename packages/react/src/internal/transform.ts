...
import { bytesToBase64, parseTransform } from "@scumble/graphics";
 ...
 * `transform` prop (every `scumble-*` node accepts it; nested transforms
 * cascade in the renderer).
 *
 * The value is first turned into a CSS `transform` string, then
 * `@scumble/graphics`'s `parseTransform` serializes it to nested FlatBuffer
